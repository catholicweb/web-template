<template>
  <footer class="border-t border-gray-700 pt-8 mt-16 pb-4" :class="navStyle == '47herri' ? '[&_*]:text-white bg-[#222831] pt-4' : 'bg-gray-100'">
    <div class="container mx-auto px-4">
      ...
      <!-- ===== Standard & Auto & Expanded: column sections ===== -->
      <template v-if="footerStyle !== 'minimal'">
        <div :class="gridClass" class="gap-8 text-center mb-8">

          <!-- Contact Column (auto: only if has data) -->
          <div v-if="showContact" :class="contactColClass">
          ...
          <div v-if="showDescription" :class="descriptionColClass">
            <h3 class="text-xl font-semibold mb-3">{{ config.title }}</h3>
            <p class="mb-2" v-if="config.description">{{ config.description }}</p>
          ...
          <div v-if="showBank" :class="bankColClass">
          ...
          <div v-if="footerStyle === 'expanded' && sitemapLinks.length" class="md:col-span-1">
        ...
        <!-- Expanded only: social icons in a full-width row -->
        <div v-if="footerStyle === 'expanded' && showSocial" class="text-center mb-6">
          <div class="flex gap-4 items-center mx-auto justify-center">
            <template v-for="(href, index) in config.social">
              <a v-if="href.includes('youtube')" target="_blank" rel="noopener noreferrer" aria-label="Youtube" :href="href" class="inline-block hover:text-red-500 transition-colors">
              ...
              <a v-else-if="href.includes('instagram')" target="_blank" rel="noopener noreferrer" aria-label="Instagram" :href="href" class="inline-block hover:text-pink-500 transition-colors">
              ...
              <a v-else-if="href.includes('twitter')" target="_blank" rel="noopener noreferrer" aria-label="Twitter/X" :href="href" class="inline-block hover:text-gray-700 transition-colors">
                <svg class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126h3.117z" /></svg>
              </a>
              <a v-else href="#" class="inline-block hover:text-accent transition-colors" @click.prevent>🔗</a>
            </template>
          </div>
        </div>
      </template>
      ...
      <!-- ===== Minimal footer ===== -->
      <div v-if="footerStyle === 'minimal'" class="text-center mb-6">
...
const hasContacts = computed(() => config.value.collaborators?.some((c) => c.phonenumber || c.email));
const hasSocial = computed(() => config.value.social?.length > 0);
const hasBank = computed(() => config.value.bank?.length > 0);
const hasDescription = computed(() => config.value.title || config.value.description);

const firstContact = computed(() => {
  if (!config.value.collaborators?.length) return null;
  return config.value.collaborators.find((c) => c.phonenumber || c.email) || config.value.collaborators[0];
});
...
// Auto mode: show only columns with data, adjust grid
const showContact = computed(() => {
  if (footerStyle.value === "standard" || footerStyle.value === "expanded") return true;
  if (footerStyle.value === "auto") return hasContacts.value;
  return false;
});

const showDescription = computed(() => {
  if (footerStyle.value === "standard" || footerStyle.value === "expanded") return true;
  if (footerStyle.value === "auto") return hasDescription.value || hasSocial.value;
  return false;
});

const showSocial = computed(() => {
  if (footerStyle.value === "expanded") return hasSocial.value;
  if (footerStyle.value === "auto") return hasSocial.value;
  return hasSocial.value;
});

const showBank = computed(() => {
  if (footerStyle.value === "standard" || footerStyle.value === "expanded") return true;
  if (footerStyle.value === "auto") return hasBank.value;
  return false;
});

// Column count and sizing for auto mode
const autoColumnCount = computed(() => {
  return [showContact.value, showDescription.value, showBank.value, footerStyle.value === "expanded"].filter(Boolean).length;
});

const gridClass = computed(() => {
  if (footerStyle.value === "auto") {
    const cols = Math.min(autoColumnCount.value, 4);
    if (cols <= 1) return "grid grid-cols-1";
    if (cols === 2) return "grid grid-cols-1 md:grid-cols-2 gap-8 text-center";
    if (cols === 3) return "grid grid-cols-1 md:grid-cols-3 gap-8 text-center";
    return "grid grid-cols-1 md:grid-cols-4 gap-8";
  }
  if (footerStyle.value === "expanded") {
    return "grid grid-cols-1 md:grid-cols-4 gap-8 text-center";
  }
  // standard
  return "grid grid-cols-1 md:grid-cols-3 gap-8 text-center";
});

// Auto: wider column for richer content
const contactColClass = computed(() => {
  if (footerStyle.value !== "auto") return "";
  const total = autoColumnCount.value;
  const contactCount = config.value.collaborators?.length || 0;
  if (total <= 2 && contactCount >= 3) return "md:col-span-2";
  return "";
});

const descriptionColClass = computed(() => {
  if (footerStyle.value !== "auto") return "";
  const total = autoColumnCount.value;
  if (total <= 2 && showSocial.value) return "md:col-span-2";
  return "";
});

const bankColClass = computed(() => {
  if (footerStyle.value !== "auto") return "";
  return "";
});
</script>
